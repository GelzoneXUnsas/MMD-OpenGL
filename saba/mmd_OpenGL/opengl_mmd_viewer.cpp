...
bool SampleMain(std::vector<std::string> &args)
{
	...
	result = ma_sound_init_from_file(&engine, "../../mmd_OpenGL/Gokuraku_Jodo.wav", 0, NULL, NULL, &sound);
	if (result != MA_SUCCESS)
	{
		return result;
	}
	...
	auto window = glfwCreateWindow(730, 1000, "OpenGL MMD Viewer", nullptr, nullptr);
	if (window == nullptr)
	{
		return false;
	}
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (!audioStart)
		{
			ma_sound_set_start_time_in_pcm_frames(&sound, ma_engine_get_time_in_pcm_frames(&engine) + (ma_uint32)(ma_engine_get_sample_rate(&engine) * 0.5));
